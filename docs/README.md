...
| [home_page.md](home_page.md) | **主页（欢迎页）**：只回答「这是什么 / 我该去哪」（欢迎卡 + 三张入口卡），`static/showcase` 静态素材、`ImageLabel` 每帧缩放 + 圆角裁剪、一格滚轮 24 帧的滚动性能 |
...
| [app_shell.md](app_shell.md) | **应用外壳**：全局字体 LXGW 文楷等宽（首选族名 + qfluentwidgets `getFont` 补丁）、FreeType 渲染后端、消息提示统一挂内容区、切页滑一张快照、主题切换的两处提速与天花板、窗口位置与大小的记忆 |
