...
> **后续更新**：① 收藏集详情页新增「加入下载」按钮，状态与下载队列联动（打开详情重置、已存在/加入后显示「已加入」禁用、队列移除后恢复，统一走 `_sync_queue_btn`）；② 详情页与表情包详情页新增「已下载过」提示（按下载目标目录是否非空判断，目录规则见 `download_runner` 的 `package_download_dir` / `collection_download_dir`）；③ 「仅看收藏集」改为**默认勾选**，且取消/勾选可实时过滤当前结果（无需重新搜索）；④ 表情包/收藏集网格均改为 `_CardGridBase` 响应式填满（随视口宽度算列数，宽屏多列、收窄自动减列）；⑤ 全部自定义控件改用 qfluentwidgets 主题化 Label（`CaptionLabel` / `StrongBodyLabel` / `BodyLabel`），并补全深色主题（全局调色板 + 主题切换即时重刷）。
...
搜索关键词 → 四列竖版海报卡片（海报 + 名称 + 类别徽标）
  ├─ 点卡片（非多选态）→ 详情页：内容分页（静态图片网格 / 动态视频播放）+ 模式选择下载
...
> 视频区后来改成了可播放的「动态视频」标签页（内嵌播放器 + 缩略图选择条），
> 折叠列表已移除，见 [collection_video.md](collection_video.md)。
